...
var CURVES_RES = [
	load("res://Scripts/Curves/Linear.tres"),
	load("res://Scripts/Curves/Exponential.tres"),
	load("res://Scripts/Curves/Inverse_S.tres")
]

export var mouse_sens = Vector2(-.3,.3) # sensitivities for each
export var gamepad_sens = Vector2(2,2) # axis + input
export var gamepad_curve = Curves.INV_S # curve analog inputs map to
export var move_speed = 4 # max move speed
...
export var jump_speed = 2 # length in frames to reach apex
export var jump_height = 0.5 # apex in meters of jump
...
func _process_input(delta):
	...
	if Input.is_action_just_pressed("ui_cancel") && mouse_control:
	...
	# WASD
	input_dir = Vector3(Input.get_action_strength("D") - Input.get_action_strength("A"), 0,
